...
/**
 * 
 * Main Spark Driver Program Class It creates Resilient Distributed Dataset for
 * a log file (external data source) It does following operation on RDD 1. Total
 * no. of lines in log file 2. Total characters in log file 3. Total no. of URL
 * in log file with HTML and GIF extension
 */
public class BigLogProcessor {
	public static void execute(String arg) {
		/* Define Spark Configuration */
		SparkConf conf = new SparkConf().setAppName("spark-study-gradle").setMaster(arg);

		/* Create Spark Context with configuration */
		JavaSparkContext sc = new JavaSparkContext(conf);

		/*
		 * Create a Resilient Distributed Dataset for a log file Each line in log file
		 * become a record in RDD
		 */
		JavaRDD<String> lines = sc.textFile(
				"D:\\MyWorkSpace\\ResearchingProjects\\spark-study-maven\\data/apache-log04-aug-31-aug-2011-nasa.log");
